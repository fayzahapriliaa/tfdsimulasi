...
def solution_B1():
    ...
    # Add early stopping to avoid overfitting and ensure we get the best model
    early_stopping = keras.callbacks.EarlyStopping(monitor='loss', patience=20, restore_best_weights=True)

    # Train the model on the data with a higher number of epochs
    model.fit(X, Y, epochs=2000, callbacks=[early_stopping])

    # Print model predictions for the given values
